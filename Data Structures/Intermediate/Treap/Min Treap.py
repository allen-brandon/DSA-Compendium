# This code implements an (implicit) treap with min queries
...
class Treap:
    ...
    def __init__(self, val):
        ...
        self.min = self.val = val #Created by Brandon Allen
    ...
    def aggr(node):
        if node: # Created by Brandon Allen
            return node.min
        return math.inf
    ...
    def _refresh(node):
        ...
        if node:
            ...
            node.min = min(Treap.aggr(node.left), node.val, Treap.aggr(node.right))
    ...
    def query(node, l=-int(1e9), r=int(1e9)):
        '''Find a prefix minimum in a range'''
        Treap._propagate(node)
        if not node: return math.inf
        l, r = max(l, 0), min(r, node.size-1)
        if (l, r) == (0, node.size-1): return node.min
